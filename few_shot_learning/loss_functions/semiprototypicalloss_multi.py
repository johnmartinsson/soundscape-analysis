...
def semi_prototypical_loss(input, target, n_support, config, semi_input=None, supp_idxs=None):
    ...
    all_query_samples = torch.zeros(0, input[0].shape[1])
    all_query_samples = torch.cat((all_query_samples, labeled_query_samples))
    
    
    if semi_input is not None:
        semi_input_og = semi_input
        unlabeled_query_samples = torch.zeros(0, semi_input[0].shape[1])
        semi_input = semi_input[0].to('cpu')
    ...
    semi_input_og = semi_input_og[1].to('cpu')
    input_og = input_og[1].to('cpu')
    
    y_clf = []
    y_clf += ([1]*len(input_og))
    y_clf += ([0]*len(semi_input_og))
    
    
    
    y_clf = torch.tensor(y_clf, dtype=torch.float32)
    x_clf = torch.cat((input_og, semi_input_og))
    x_clf = torch.reshape(x_clf, (-1,))
    
    clf_loss_f = torch.nn.BCELoss()
    clf_loss = clf_loss_f(x_clf, y_clf)
    
    
    return (loss_val.mean() + clf_loss), torch.tensor(0)
